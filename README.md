...
This repository contains an implementation of a **Rainbow DQN** agent, designed to satisfy the requirements of the 2025 RL Independent Study Final Project. The agent integrates features from three distinct categories of reinforcement learning advancements—Algorithm Specific, Algorithm Agnostic, and Adversarial —to solve discrete control environments.
...
* **Magnetic Mirror Descent (MMD):** A regularization term that tethers the online network weights to the target network weights, preventing catastrophic forgetting or divergence.
...
* **Leduc-v0:** Discrete imperfect information poker (Optional).
...
├── src/
│   ├── agent.py        # The RainbowAgent logic (C51 projection, loss calc)
...
Do not attempt to run this without the proper vessel.
...
pip install minigrid gym-leduc
```
...
To enable or disable specific environments, comment/uncomment the relevant keys in `src/config.py`. Do not try to run `HalfCheetah` (Continuous) with this Discrete agent, or it will crash.
